...
restaurant = merged_df[(merged_df['categories'].str.contains('restaurant', case=False, na=False))]

# Calculate the number of reviews for each restaurant
num_reviews_per_restaurant = restaurant.groupby('business_id')['stars'].count()

# Calculate the average rating for each restaurant
avg_rating_per_restaurant = restaurant.groupby('business_id')['stars'].mean()
